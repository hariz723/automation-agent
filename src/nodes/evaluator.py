...
class EvaluationSchema(BaseModel):
    ...
    is_successful: bool = Field(description="True if the step output satisfies the expected criteria, False otherwise")
    critique: str = Field(description="Detailed critique explaining what succeeded or what failed")
    recommendation: str = Field(description="One of: 'continue' (proceed to next step), 'retry' (try again), or 'replan' (alter remaining plan)")
...
"""
    ...
    try:
        ...
        recommendation = eval_res.recommendation.lower().strip()
    except Exception:
