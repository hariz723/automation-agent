...
class ReplannedSteps(BaseModel):
    remaining_steps: list[StepSchema] = Field(
        description="Updated sequence of remaining steps to complete the task"
    )
...
"""
    ...
    try:
        ...
        res: ReplannedSteps = structured_llm.invoke(
            [
                SystemMessage(
                    content="You are an adaptive planning agent that revises task steps when blockers occur."
                ),
